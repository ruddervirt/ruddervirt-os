...
func TestFilterLiveManifest(t *testing.T) {
	...
	t.Run("NotFound drops the doc, live objects are kept", func(t *testing.T) {
		r := &fakeRunner{respond: func(name string, args []string) commandOutcome {
			...
			if strings.Contains(line, "absent") {
				return commandOutcome{out: []byte(`Error from server (NotFound): configmaps "absent" not found`), err: errFake}
			}
			return commandOutcome{out: []byte("configmap/exists")}
		}}
		ch := make(chan tea.Msg, 100)
		var live string
		var err error
		withFakeRunner(r, func() { live, err = filterLiveManifest(ch, "/usr/local/bin/kubectl", []string{docA, docB}) })
		if err != nil {
	...
	t.Run("a non-NotFound error aborts the whole run", func(t *testing.T) {
		r := &fakeRunner{respond: func(name string, args []string) commandOutcome {
			return commandOutcome{out: []byte("Error from server: internal error"), err: errFake}
		}}
		ch := make(chan tea.Msg, 100)
		var err error
		withFakeRunner(r, func() { _, err = filterLiveManifest(ch, "/usr/local/bin/kubectl", []string{docA}) })
		if err == nil {
...
func TestFindOrphanedHelmChartCRs(t *testing.T) {
	...
	r := &fakeRunner{respond: func(name string, args []string) commandOutcome {
		return commandOutcome{out: []byte(listJSON)}
	}}
	var crs []helmChartCR
	var err error
	withFakeRunner(r, func() { crs, err = findOrphanedHelmChartCRs("/usr/local/bin/kubectl", "aileron", "ruddervirt-system") })
	if err != nil {
...
func TestRetireHelmChartCR(t *testing.T) {
	t.Run("drops finalizers before deleting, in order, then deletes the job", func(t *testing.T) {
		...
		var err error
		withFakeRunner(r, func() { err = retireHelmChartCR(ch, "/usr/local/bin/kubectl", cr) })
		if err != nil {
...
func TestDeleteImmutableSelectorWorkloads(t *testing.T) {
	...
	var deleted []string
	r := &fakeRunner{respond: func(name string, args []string) commandOutcome {
		if cmdContains(name, args, "get", "deployments,statefulsets,daemonsets") {
			return commandOutcome{out: []byte(listJSON)}
		}
		if cmdContains(name, args, "delete") {
	...
	ch := make(chan tea.Msg, 100)
	var err error
	withFakeRunner(r, func() { err = deleteImmutableSelectorWorkloads(ch, "/usr/local/bin/kubectl", "ruddervirt-system") })
	if err != nil {
...
func TestAdoptAileronStep(t *testing.T) {
	...
	t.Run("no release and no orphaned CR is a clean no-op", func(t *testing.T) {
		r := &fakeRunner{respond: func(name string, args []string) commandOutcome {
			...
			case cmdContains(name, args, "systemctl", "is-active", "k3s.service"):
				return commandOutcome{} // k3s is running
			case cmdContains(name, args, "wait", "deployment.apps/aileron"):
				return commandOutcome{} // aileron is ready
			case cmdContains(name, args, "get", "secret"):
				return commandOutcome{out: []byte(emptySecretList)}
			case cmdContains(name, args, "get", "helmchart"):
				return commandOutcome{out: []byte(emptyHelmChartList)}
			}
			t.Errorf("unexpected call: %s %v", name, args)
			return commandOutcome{}
		}}
		ch := make(chan tea.Msg, 100)
		withFakeRunner(r, func() { adoptAileronStep(Config{}, ch) })
	...
	t.Run("manifest missing vncgateway is a hard error", func(t *testing.T) {
		...
		r := &fakeRunner{respond: func(name string, args []string) commandOutcome {
			...
			case cmdContains(name, args, "wait", "deployment.apps/aileron"):
				return commandOutcome{} // aileron is ready
			case cmdContains(name, args, "get", "secret"):
				return commandOutcome{out: []byte(secretList)}
			}
			t.Errorf("unexpected call: %s %v", name, args)
			return commandOutcome{}
		}}
	...
	t.Run("full happy-path adoption runs the whole sequence", func(t *testing.T) {
		...
		r := &fakeRunner{respond: func(name string, args []string) commandOutcome {
			...
			case cmdContains(name, args, "wait", "deployment.apps/aileron"):
				return commandOutcome{} // aileron is ready
			case cmdContains(name, args, "get", "secret", "status=deployed"):
				return commandOutcome{out: []byte(secretList)}
			case cmdContains(name, args, "get", "Deployment", "aileron-vncgateway"):
				return commandOutcome{out: []byte("deployment.apps/aileron-vncgateway")}
			case cmdContains(name, args, "annotate"):
				sawAnnotate = true
				return commandOutcome{}
			case cmdContains(name, args, "delete", "secret"):
				sawDeleteSecrets = true
				return commandOutcome{}
			case cmdContains(name, args, "get", "helmchart"):
				return commandOutcome{out: []byte(emptyHelmChartList)}
			case cmdContains(name, args, "get", "deployments,statefulsets,daemonsets"):
				sawDeleteWorkloads = true
				return commandOutcome{out: []byte(`{"items":[]}`)}
			case cmdContains(name, args, "label"):
				return commandOutcome{}
			}
			t.Errorf("unexpected call: %s %v", name, args)
			return commandOutcome{}
...
// test if none arrived.
func lastStepDone(t *testing.T, ch chan tea.Msg) stepDoneMsg {
	t.Helper()
