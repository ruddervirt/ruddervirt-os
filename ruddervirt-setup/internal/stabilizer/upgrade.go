...
// classifyPins/buildUpgradePatch) so a chart-version change made through
// ruddervirt-setup (package main's stabilizer_version_tui.go) is bound by
// exactly the rules the cloud UI's over-the-wire upgrade RPC enforces - see
// internal/stabilizer/settings/cli.go's header comment on why nothing
// downstream re-checks a write to this HelmChart resource.
//
...
//     there is no clear_image_pins escape hatch here. Only a redundant
//     release pin (the common case: install.sh used to write these on every
//     install) is auto-masked, silently, same as upstream.
...
//     touches spec.version (plus any masked image pins the version change
//     requires).
...
// (k3s/KubeVirt/CDI/Aileron release tags): chart versions on the HelmChart
// resource are never "v"-prefixed.
...
// unparseable current version (empty spec.version, or a chart installed by
// some other path) skips both checks - nothing principled to compare
// against, and refusing would strand exactly the cluster most in need of
// moving to a known version.
...
// a developer's override (refuse). The distinction matters because silently
// masking a dev pin would move the chart and every other image while
...
// change. Aileron's release tags and the stabilizer chart's version are kept
// in strict lockstep by that project's CI (the stabilizer chart bundles
// aileron as a subchart pinned to the matching version, both repos' releases
// cut together) - so aileron's release list is exactly the set of valid
// stabilizer spec.version targets, with no separate stabilizer release feed
// to fetch. Filtered to versions at or above the currently declared one
// (never offer a downgrade from here, same convention as
// versions.k3s/versions.aileron's options funcs), displayed "v"-prefixed for
// consistency with every other version list in this app; the "v" is
// stripped only when building the patch (PlanStabilizerVersionUpgrade takes
// the already-stripped value).
...
func preflightStabilizerVersionUpgrade(state *settings.StabilizerSettingsState) error {
...
// the durable-record-before-patching step: the caller performs the write
// itself via kubectl (stabilizer_version_tui.go), and there is no
// ZoneReporter on the box side to record against.
func PlanStabilizerVersionUpgrade(state *settings.StabilizerSettingsState, targetVersion string) (patch []byte, clearedPins []string, err error) {
	...
	for _, dotted := range cleared {
		...
		settings.SetByPath(values, dotted, "")
	}
