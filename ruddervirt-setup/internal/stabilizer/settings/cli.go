...
import (
	...
	// Aliased: LoadStabilizerSettingsState below takes a parameter named
	// exec (a KubectlExecFunc), which would otherwise shadow this package
...
//
// Deliberately does NOT go through runPrivileged/sudo like the rest of this
// package's kubectl calls: the k3s kubeconfig at /etc/rancher/k3s/k3s.yaml
// is root-only, so an unelevated `admin` shell can't reach the API server
// without an exported KUBECONFIG or running as root - expected to be the
// single most common failure here, so friendlyKubectlError names it
...
// DIRECTLY - nothing else validates it. See validate.go's
// ParseStabilizerSettingValue for why that makes this tool the only
...
// stabilizerNamespace mirrors internal/stabilizer.StabilizerNamespace's
// value. Duplicated here (a plain literal, not imported) since
// stabilizer_upgrade.go (internal/stabilizer) needs to import THIS package
// for StabilizerSettingsState - importing internal/stabilizer here too
// would create a cycle. Same idea as kubectlBinPath above.
const stabilizerNamespace = "ruddervirt-system"

const (
	// defaultStabilizerHelmChartNamespace/Name are install.sh's own
	// defaults for the HelmChart resource that installs stabilizer -
	// --chart-namespace and the release name can change them, so these are
	// only the fallback when the running Deployment's own
	// HELM_CHART_NAMESPACE/HELM_CHART_NAME env vars (written into
	// spec.values.selfUpgrade by install.sh for exactly this reason) aren't
	// available.
	...
	// (ruddervirt-system, internal/stabilizer's stabilizer.go).
...
// comment for why. Stays exported so internal/tui's "Stabilizer Settings"
// screen (once it moves there) can inject its own privileged variant
// through the same seam settingsKubectl uses.
...
func settingsKubectl(args ...string) ([]byte, error) {
	...
	defer cancel()
	return execpkg.DefaultRunner.CombinedOutput(ctx, kubectlBinPath, args...)
}
...
// failure into specific guidance: an unelevated shell failing to read the
// root-only k3s kubeconfig is expected to be the single most common failure
// here, so it's worth naming explicitly rather than surfacing kubectl's raw
// (often cryptic) error text.
...
// throughout this codebase (k8sNodeList, internal/k3s's k3s.go;
// k8sSecretGetJSON, internal/stabilizer's adopt.go) rather than a
// client-go/apimachinery dependency.
...
type k8sHelmChartGetJSON struct {
	Spec struct {
		...
		// chart-version-upgrade flow (internal/stabilizer's upgrade.go) -
		// see LoadStabilizerSettingsState's effectiveValues merge below for
		// why ValuesContent can't just be ignored the way it was before.
...
// applyStabilizerSettingsChanges need, gathered by
// LoadStabilizerSettingsState in exactly three kubectl reads.
type StabilizerSettingsState struct {
	...
	// resolve: spec.valuesContent (the previous run's user-supplied values,
	// carried forward verbatim by install.sh) merged with spec.values (which
	// wins) on top - mirrors stabilizer's own effectiveValues/mergeInto
	// (internal/handler/upgrade.go), since comparing against spec.values
	// alone would miss anything only ever written into valuesContent. A
	// setting's path missing from this tree means "no explicit value
	// declared; the chart default applies" - NOT "off"/zero. spec.set is
	// deliberately never merged in - see buildUpgradePatch's doc comment
	// (internal/stabilizer's upgrade.go) for why.
	...
	// (not a second kubectl round trip) because internal/stabilizer's
	// upgrade.go needs all three for the same checks stabilizer's own
	// preflight (internal/handler/upgrade.go) applies.
	...
	// this box-side tool applies exactly the guard the agent would apply to
	// the same request over NATS.
...
// HelmChart resource's actual namespace/name), the HelmChart resource's
// declared spec.values, and whether a release operation is already in
// flight.
//
// exec abstracts HOW each kubectl call runs, so this parsing/assembly logic
// is shared between two exec strategies: the CLI subcommand (settingsKubectl,
// deliberately no sudo - see this file's header comment) and the interactive
// TUI's "Stabilizer Settings" screen (package main's
// stabilizer_settings_tui.go), which uses the same interactive-sudo
...
func LoadStabilizerSettingsState(exec KubectlExecFunc) (*StabilizerSettingsState, error) {
	...
	if err != nil {
		...
		return nil, execpkg.WrapCmdErr(depOut, err)
	}
	...
	if err != nil {
		...
		return nil, execpkg.WrapCmdErr(valuesOut, err)
	}
	...
	default:
		return nil, execpkg.WrapCmdErr(jobOut, jobErr)
	}
...
// against: the value already DECLARED on the HelmChart resource if present
// (even if its rollout hasn't landed yet), else the currently APPLIED
// value. Mirrors stabilizer's own resolveSettings (internal/handler/
// upgrade.go), so a second identical --set while a rollout is still
...
func applyStabilizerSettingsChanges(state *StabilizerSettingsState, setArgs []string, skipConfirm bool) int {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "Error:", friendlyKubectlError(execpkg.WrapCmdErr(out, err)))
		return 1
	}
