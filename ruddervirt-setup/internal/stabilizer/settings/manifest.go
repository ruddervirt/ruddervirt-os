...
// Package settings implements `ruddervirt-setup settings` - a non-interactive
// CLI subcommand (dispatched in main.go before the TUI ever starts) that
// reads and changes stabilizer's configurable settings directly on the
// HelmChart resource, without needing helm or kubectl expertise - plus the
// shared parsing/validation logic the interactive TUI's "Stabilizer
// Settings" screen (stabilizer_settings_tui.go) reuses.
package settings
...
// This file, cli.go, and validate.go implement the settings CLI/TUI shared
// logic described in the package doc comment above.
//
...
// Instead it's committed here and bumped by hand whenever the pinned
// stabilizer version (versions.DefaultStabilizerVersion, internal/versions)
// changes - the same "hand-maintained, go:embed'd from the package
// directory" pattern default-versions.yaml itself already uses.
//
...
// present-but-zero-value YAML key still leaves this field non-nil, while a
// genuinely absent key leaves it nil. That nil-vs-present distinction is
// what callers need: "unlimited" is a deliberate choice a setting either
// offers or doesn't, never "unset".
...
// init() plays: a parse failure here means the vendored file itself is
// broken, a build-time bug that should fail loudly rather than leave the
...
func init() {
	...
	if err := yaml.Unmarshal(stabilizerSettingsYAML, &f); err != nil {
		panic(fmt.Sprintf("ruddervirt-setup/internal/stabilizer/settings/stabilizer-settings.yaml failed to parse: %v", err))
	}
	if len(f.Settings) == 0 {
		panic("ruddervirt-setup/internal/stabilizer/settings/stabilizer-settings.yaml declares no settings")
	}
	...
	for _, d := range f.Settings {
		...
		if d.Key == "" || d.Path == "" || d.Env == "" || d.Component == "" || d.Summary == "" {
			panic(fmt.Sprintf("ruddervirt-setup/internal/stabilizer/settings/stabilizer-settings.yaml: setting %q is missing a required field (key/path/env/component/summary)", d.Key))
		}
		if d.Component != "stabilizer" && d.Component != "aileron" {
			panic(fmt.Sprintf("ruddervirt-setup/internal/stabilizer/settings/stabilizer-settings.yaml: setting %q has unknown component %q", d.Key, d.Component))
		}
		switch d.Type {
		case StabilizerSettingBool, StabilizerSettingInt, StabilizerSettingQuantity:
		default:
			panic(fmt.Sprintf("ruddervirt-setup/internal/stabilizer/settings/stabilizer-settings.yaml: setting %q has unknown type %q", d.Key, d.Type))
		}
		if d.Type == StabilizerSettingInt && d.Max <= d.Min {
			panic(fmt.Sprintf("ruddervirt-setup/internal/stabilizer/settings/stabilizer-settings.yaml: setting %q has max <= min (%d <= %d)", d.Key, d.Max, d.Min))
		}
		if _, dup := byKey[d.Key]; dup {
			panic(fmt.Sprintf("ruddervirt-setup/internal/stabilizer/settings/stabilizer-settings.yaml: duplicate setting key %q", d.Key))
		}
		byKey[d.Key] = d
	}
...
// (base) with spec.values (wins) into one effective view; see
// LoadStabilizerSettingsState (cli.go).
