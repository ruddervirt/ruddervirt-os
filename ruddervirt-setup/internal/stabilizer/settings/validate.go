...
// maxQuantityLen exactly - a resource quantity is never anywhere close to
// this long ("1500Gi" is 6 characters), rejecting an oversized payload
// before it's even handed to resource.ParseQuantity.
...
// spec.values with nothing else checking it. A bad value here isn't
...
// generic map[string]any tree: JSON numbers arrive as float64) into the
// same Go type ParseStabilizerSettingValue returns for d, so declared and
// applied/requested values can be compared directly. ok is false if raw
// isn't a legal encoding of d's type (e.g. a non-whole float64 for an int
// setting) - callers must treat that as "the resource disagrees with the
// manifest", not silently coerce it.
...
// byte-for-byte - a merge patch that changes a quantity's spelling without
// changing its value would still bounce the release for nothing, exactly
// the redundant-write case this exists to catch. An empty quantity string
// ("unlimited") only equals another empty string.
